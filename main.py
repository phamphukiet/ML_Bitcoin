...
def get_or_create_scalers(train_df):
    scaler_x, scaler_y = scaler_utils.get_fresh_scalers(
        train_df[FEATURE_COLS], train_df[[TARGET_COL]],  # 👈 X và y tách riêng
...
def get_model(x_train):
    ...
    if latest_version > 0:
        ...
        # ⚡ Cho phép chọn fine-tune hoặc reset weight
        RESET_MODEL = True  # 👈 config nhỏ, có thể đưa vào config.py
        if RESET_MODEL:
            print(f"🔄 Loading kiến trúc từ v{latest_version} và reset weight...")
            # model = model_utils.load_existing_model_with_reset(prev_path, (x_train.shape[1], x_train.shape[2]))
